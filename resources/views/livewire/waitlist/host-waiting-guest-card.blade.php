<flux:card size="sm">
    <div class="flex items-start justify-between gap-3">
        ...
        <flux:badge size="sm" icon="clock">{{ __('waitlist.position') }} {{ $item->position }}</flux:badge>
    </div>
    <div class="mt-2 flex flex-wrap gap-1">
        ...
        @if($item->user?->phone_verified)
            <flux:badge size="sm" icon="check-circle">{{ __('waitlist.host.phone_verified') }}</flux:badge>
        @endif
        @if($item->user?->identity_verified)
            <flux:badge size="sm" icon="check-circle">{{ __('waitlist.host.profile_verified') }}</flux:badge>
        @endif
    </div>
</flux:card>
