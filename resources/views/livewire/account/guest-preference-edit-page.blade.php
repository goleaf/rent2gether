<div class="mx-auto max-w-3xl space-y-5">
    <section class="space-y-2">
        <flux:heading size="xl" level="1">{{ __('preferences.edit.heading') }}</flux:heading>
    ...
    <form wire:submit="save" class="space-y-5">
        <flux:card class="space-y-4">
            <flux:heading size="sm">{{ __('preferences.sections.budget') }}</flux:heading>
            @include('livewire.account.partials.guest-preference-section', ['section' => 'budget'])
        </flux:card>

        <flux:card class="space-y-4">
            <flux:heading size="sm">{{ __('preferences.sections.place') }}</flux:heading>
            @include('livewire.account.partials.guest-preference-section', ['section' => 'place'])
        </flux:card>

        <flux:card class="space-y-4">
            <flux:heading size="sm">{{ __('preferences.sections.comfort') }}</flux:heading>
            @include('livewire.account.partials.guest-preference-section', ['section' => 'comfort'])
        </flux:card>

        <flux:card class="space-y-4">
            <flux:heading size="sm">{{ __('preferences.sections.lifestyle') }}</flux:heading>
