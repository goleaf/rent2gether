    ...
    <section class="space-y-3">
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 space-y-2">
                <flux:badge color="emerald">{{ __('notifications.page.eyebrow') }}</flux:badge>
                <flux:heading size="xl" level="1">{{ __('notifications.page.title') }}</flux:heading>
            ...
            @if($this->unreadCount > 0)
                <flux:button size="sm" variant="ghost" wire:click="markAllAsRead" class="shrink-0 data-loading:opacity-70">
        ...
        @forelse($this->items as $notification)
            <flux:card @class([
                    ...
                    <div class="min-w-0 space-y-1">
                        <div class="flex flex-wrap items-center gap-2">
                            @if($notification->isUnread())
                                <flux:badge size="sm" color="emerald">{{ __('notifications.status.unread') }}</flux:badge>
                            @else
                                <flux:badge size="sm">{{ __('notifications.status.read') }}</flux:badge>
                            @endif
                ...
                <div class="flex flex-col gap-2 sm:flex-row">
                    ...
                    @if($notification->isUnread())
                        <flux:button type="button" wire:click="markAsRead('{{ $notification->id }}')" variant="ghost" class="w-full data-loading:opacity-70 sm:w-auto">
        ...
        @empty
            <flux:card class="space-y-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="space-y-1">
                        <flux:heading size="lg">{{ __('notifications.page.empty_title') }}</flux:heading>
                        <flux:text class="text-zinc-600 dark:text-zinc-400">
