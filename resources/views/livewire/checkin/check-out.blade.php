<div class="mx-auto max-w-3xl space-y-5 px-4 py-4 pb-32 sm:px-6">
    <section class="space-y-3">
        <flux:badge color="emerald">{{ __('booking.checkout.eyebrow') }}</flux:badge>
        <div class="space-y-2">
            <flux:heading size="xl" level="1">{{ __('booking.checkout.title') }}</flux:heading>
    ...
    <form wire:submit="submit" class="space-y-4">
        <flux:card class="space-y-4">
            <div class="space-y-1">
                <flux:heading size="lg">{{ __('booking.checkout.checklist') }}</flux:heading>
                <flux:text size="sm" class="text-zinc-600 dark:text-zinc-400">
