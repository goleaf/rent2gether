<form wire:submit="save" class="space-y-5">
    <flux:card class="space-y-4">
        <div>
            <flux:heading size="lg">{{ __('sleeping_place.steps.comfort.title') }}</flux:heading>
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            @foreach(['mattressType', 'mattressFirmness', 'mattressCondition', 'mattressNewness', 'mattressThicknessCm'] as $field)
                <flux:field>
                    ...
                    <flux:input @if($field === 'mattressThicknessCm') type="number" inputmode="numeric" @endif wire:model.blur="{{ $field }}" />
