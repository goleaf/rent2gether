<x-ui.page class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <flux:card class="space-y-3">
            <flux:heading size="sm">{{ __('booking.guest') }}</flux:heading>
    ...
    <div class="flex flex-wrap gap-3">
        @if($booking->status->value === 'pending_host')
            ...
            <flux:button wire:click="$set('showRejectModal', true)" variant="danger">{{ __('host.manage_booking.reject') }}</flux:button>
        @endif

        @if(in_array($booking->status->value, ['confirmed', 'paid', 'ready_for_checkin']))
            <flux:button wire:click="confirmCheckIn" variant="primary">{{ __('host.manage_booking.confirm_checkin') }}</flux:button>
