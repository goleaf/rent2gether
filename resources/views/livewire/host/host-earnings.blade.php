<div class="max-w-4xl mx-auto space-y-6">
    <flux:heading size="xl">{{ __('Earnings') }}</flux:heading>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <flux:card class="text-center">
            ...
            <flux:text size="sm" class="text-zinc-500">{{ __('This week') }}</flux:text>
        </flux:card>
        <flux:card class="text-center">
        ...
        <flux:card class="text-center">
            ...
            <flux:text size="sm" class="text-zinc-500">{{ __('This year') }}</flux:text>
        </flux:card>
    </div>

    <div class="space-y-3">
        <flux:heading size="lg">{{ __('Recent Payouts') }}</flux:heading>
        @forelse($this->recentPayouts as $payout)
            <flux:card class="flex items-center justify-between">
                <div>
                    ...
                    <flux:text size="sm" class="text-zinc-500">{{ $payout->created_at->format('M d, Y') }}</flux:text>
                </div>
        ...
        @empty
            <flux:card>
                <flux:text class="text-center text-zinc-500 py-4">{{ __('No payouts yet.') }}</flux:text>
            </flux:card>
        @endforelse
    </div>
</div>
