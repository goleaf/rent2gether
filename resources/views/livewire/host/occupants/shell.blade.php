<section class="space-y-4">
    <flux:card class="space-y-3">
        <div class="flex items-start justify-between gap-3">
            <div class="space-y-1">
                <flux:badge color="zinc">{{ __('current_occupants.title') }}</flux:badge>
                <flux:heading size="lg">{{ __('current_occupants.sections.'.$section) }}</flux:heading>
            </div>
    ...
    <flux:card class="space-y-3">
        <flux:heading size="base">{{ __('current_occupants.filters.title') }}</flux:heading>
        <div class="flex flex-wrap gap-2">
            @foreach (['all', 'check_ins_today', 'check_outs_today', 'leaving_soon', 'checkout_overdue', 'payment_pending', 'complaints', 'needs_extension', 'needs_checkout', 'needs_cleaning'] as $filter)
                <flux:badge color="zinc">{{ __('current_occupants.filters.'.$filter) }}</flux:badge>
            @endforeach
        </div>
    </flux:card>
    ...
    @if ($summary->currentCount === 0)
        <flux:card class="space-y-2">
            <flux:heading size="base">{{ __('current_occupants.empty.title') }}</flux:heading>
            <flux:text size="sm" class="text-zinc-600 dark:text-zinc-400">
    ...
    @else
        <flux:card class="space-y-3">
            <flux:heading size="base">{{ __('current_occupants.cards.title') }}</flux:heading>
    ...
    <flux:card class="space-y-2">
        <flux:heading size="base">{{ __('current_occupants.actions.title') }}</flux:heading>
