<x-ui.page>
    ...
    <flux:card class="space-y-4">
        <div class="space-y-1">
            <flux:heading size="sm">{{ __('host.sleeping_places.bulk.title') }}</flux:heading>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <flux:field>
                ...
                <flux:input type="number" inputmode="numeric" wire:model.blur="bulkCount" />
                <flux:error name="bulkCount" />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('host.sleeping_places.fields.bulk_title_prefix') }}</flux:label>
                <flux:input wire:model.blur="bulkTitlePrefix" />
            ...
            <flux:field>
                ...
                <flux:input type="number" inputmode="decimal" step="0.01" wire:model.blur="bulkBasePrice" />
                <flux:error name="bulkBasePrice" />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('host.sleeping_places.fields.bulk_currency') }}</flux:label>
                <flux:input maxlength="3" wire:model.blur="bulkCurrency" />
                <flux:error name="bulkCurrency" />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('host.sleeping_places.fields.bulk_min_nights') }}</flux:label>
                <flux:input type="number" inputmode="numeric" wire:model.blur="bulkMinNights" />
                <flux:error name="bulkMinNights" />
            </flux:field>
            <flux:field>
                <flux:label>{{ __('host.sleeping_places.fields.bulk_max_guests') }}</flux:label>
                <flux:input type="number" inputmode="numeric" wire:model.blur="bulkMaxGuests" />
                <flux:error name="bulkMaxGuests" />
            </flux:field>
        </div>

        <flux:button type="button" variant="primary" wire:click="bulkCreate" class="w-full data-loading:opacity-70">
    ...
    <div class="space-y-4">
                ...
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0 space-y-1">
                        <div class="flex flex-wrap items-center gap-2">
                            <flux:heading size="sm">{{ $place['title'] }}</flux:heading>
                            <flux:badge size="sm">{{ $place['status_label'] }}</flux:badge>
                        </div>
        ...
        @empty
            <flux:card class="space-y-3 text-center">
                <flux:heading size="sm">{{ __('host.sleeping_places.empty_title') }}</flux:heading>
