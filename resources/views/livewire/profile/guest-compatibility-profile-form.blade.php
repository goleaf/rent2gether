        ...
        <div class="space-y-1">
            <flux:heading size="sm">
                <span class="inline-flex min-w-0 items-center gap-2">
                    ...
                    <span class="min-w-0">{{ __('compatibility.prompt_title') }}</span>
                </span>
            </flux:heading>
            <flux:text size="sm" class="text-zinc-500 dark:text-zinc-400">{{ __('compatibility.prompt_helper') }}</flux:text>
        </div>

        <div class="grid gap-3 sm:grid-cols-2">
            @foreach($this->promptFieldOptions() as $promptField)
                <flux:field variant="inline" wire:key="compatibility-prompt-{{ $promptField['property'] }}">
                    <flux:checkbox wire:model.change="{{ $promptField['property'] }}" />
                    <flux:label>
                        <span class="inline-flex min-w-0 items-center gap-1.5">
                            <flux:icon name="{{ $promptField['icon'] }}" variant="mini" class="size-4 shrink-0 text-sky-500/80 dark:text-sky-300/80" />
                            <span class="min-w-0">{{ $promptField['label'] }}</span>
                        </span>
                    </flux:label>
                    <flux:error name="{{ $promptField['property'] }}" />
                </flux:field>
            @endforeach
        </div>
    </flux:card>
