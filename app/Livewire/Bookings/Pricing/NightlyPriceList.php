...
class NightlyPriceList extends Component
{
    #[Locked]
    public int $quoteId;
    ...
    public function render(): View
    {
        $quote = BookingQuote::query()
            ...
            ->with(['lines' => fn ($query) => $query
                ->select(['id', 'booking_quote_id', 'line_type', 'label_key', 'date', 'amount', 'currency', 'sort_order'])
                ->whereIn('line_type', ['night', 'weekday_night', 'weekend_night', 'holiday_night', 'date_override'])
                ->orderBy('sort_order')])
            ->findOrFail($this->quoteId);

        return view('livewire.bookings.pricing.nightly-price-list', [
