...
class HostRoomFlowTest extends TestCase
{
    ...
    public function test_host_can_update_room_with_translations(): void
    {
        ...
        Livewire::actingAs($host)
            ...
            ->set('bedsCount', 2)
            ->set('maxGuests', 2)
            ->set('descriptionEn', 'A calm shared room with two sleeping places.')
            ->set('descriptionRu', 'Спокойная общая комната с двумя спальными местами.')
            ->set('notesEn', 'Please keep the window closed at night.')
            ->set('notesRu', 'Пожалуйста, закрывайте окно ночью.')
            ->call('publish')
            ->assertHasNoErrors()
            ->assertRedirect();
    ...
    public function test_beds_count_can_generate_sleeping_place_drafts(): void
    {
        ...
        Livewire::actingAs($host)
            ...
            ->set('bedsCount', 3)
            ->set('maxGuests', 3)
            ->set('descriptionEn', 'A room prepared for generated sleeping places.')
            ->set('descriptionRu', 'Комната подготовлена для созданных спальных мест.')
