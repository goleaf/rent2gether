...
class HostPropertyWizardTest extends TestCase
{
    ...
    public function test_translated_content_is_saved(): void
    {
        ...
        Livewire::actingAs($host)
            ...
            ->set('titleEn', 'Calm bed near the old town')
            ->set('titleRu', 'Спокойное место рядом со старым городом')
            ->set('summaryEn', 'A simple shared stay with clear rules.')
            ->set('summaryRu', 'Простое проживание с понятными правилами.')
            ->set('descriptionEn', 'Guests get one sleeping place inside a quiet apartment.')
            ->set('descriptionRu', 'Гость получает одно спальное место в спокойной квартире.')
            ->set('whatToKnowEn', 'The address is shown after booking.')
            ->set('whatToKnowRu', 'Адрес показывается после бронирования.')
            ->set('suitableForEn', 'Short work trips.')
            ->set('suitableForRu', 'Короткие рабочие поездки.')
            ->set('notSuitableForEn', 'Late parties.')
            ->set('notSuitableForRu', 'Поздние вечеринки.')
            ->call('saveStep')
            ->assertHasNoErrors();
    ...
    public function test_property_status_moves_from_draft_to_active_on_review_save(): void
    {
        ...
        Livewire::actingAs($host)
            ...
            ->call('nextStep')
            ->set('titleEn', 'Quiet house with shared beds')
            ->set('titleRu', 'Тихий дом со спальными местами')
            ->set('summaryEn', 'A calm place for short stays.')
            ->set('summaryRu', 'Спокойное место для коротких поездок.')
            ->call('nextStep')
            ->set('amenityIds', [$amenity->id])
            ->call('nextStep')
            ->set('ruleIds', [$rule->id])
